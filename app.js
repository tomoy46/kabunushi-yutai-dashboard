(function (root) {
  ...
  const enrich = (benefit, market = {}) => {
    const tier = benefit.benefit_tiers?.[0] || {shares:null,description:'優待内容は公式確認未完了',annual_value_yen:null}; const price = market.price ?? benefit.sample_price ?? null;
  ...
  const verifiedLabel = value => value ? `公式確認日 ${value}` : '公式確認日 未確認';
  ...
  function render() {
    ...
    $('tableBody').innerHTML=list.map(x=>`<tr><td class="company"><span class="code">${x.code} · ${x.market}</span><b>${x.name}</b>${statusBadge(x)}<span>${x.industry}</span></td><td><b>${priceLabel(x)}</b><span class="sub">${x.tier.shares==null?'必要株数 未確認':x.tier.shares+'株'} / ${x.investment==null?'必要投資額 未確認':'¥'+yen.format(x.investment)}<br>${updatedLabel(x.price_at)}</span></td><td class="benefit"><span class="pill">${x.category}</span><br>${x.tier.description}</td><td>${x.record_months.map(m=>m+'月').join('・')}</td><td>${yieldText(x,x.benefitYield)}</td><td>${x.dividendYield==null?'データなし':pct(x.dividendYield)+(x.dividendSample?'（参考値）':'')}</td><td class="yield">${yieldText(x,x.totalYield)}</td><td>${favoriteButton(x.code)}<button class="details-button" data-detail="${x.code}">詳細</button></td></tr>`).join('');
    $('cards').innerHTML=list.map(x=>`<article class="card"><div class="card-head"><div><span class="code">${x.code} · ${x.market}</span><h3>${x.name}</h3>${statusBadge(x)}<span class="sub">${x.industry} / 権利 ${x.record_months.join('・')}月</span></div>${favoriteButton(x.code)}</div><div class="card-benefit"><span class="pill">${x.category}</span> ${x.tier.description}<div class="sub">${x.tier.shares==null?'必要株数 未確認':x.tier.shares+'株〜'} / ${x.investment==null?'必要投資額 未確認':'必要投資額 ¥'+yen.format(x.investment)}</div></div><div class="card-numbers"><div><b>${yieldText(x,x.benefitYield)}</b><span>優待利回り</span></div><div><b>${x.dividendYield==null?'—':pct(x.dividendYield)}</b><span>配当利回り${x.dividendSample?'（参考値）':''}</span></div><div><b class="yield">${yieldText(x,x.totalYield)}</b><span>総合利回り</span></div></div><div class="card-bottom"><span class="sub">${priceLabel(x)} / ${updatedLabel(x.price_at)}</span><button class="details-button" data-detail="${x.code}">詳細を見る →</button></div></article>`).join('');
  }
  function showDetail(code){const x=items.find(i=>i.code===code);$('detailContent').innerHTML=`<div class="detail-title"><span class="code">${x.code} · ${x.market}</span><h2>${x.name}</h2>${statusBadge(x)}<span>${x.industry} / ${verifiedLabel(x.official_verified_at)}</span></div><div class="detail-grid"><div><span>${x.priceSample?'サンプル株価':'現在株価'}</span><b>${x.price==null?'株価 未取得':'¥'+yen.format(x.price)}</b><small>${updatedLabel(x.price_at)}</small></div><div><span>${x.dividendSample?'サンプル予想年間配当':'予想年間配当'}（1株）</span><b>${x.dividend==null?'データなし':'¥'+yen.format(x.dividend)}</b></div><div><span>長期保有条件</span><b>${x.long_term_condition||'なし'}</b></div></div><p>${x.change_or_abolition_note||'変更・廃止情報なし'}</p>${x.last_record_date?`<p>最終基準日：${x.last_record_date} / 廃止日：${x.abolished_at}</p>`:''}<h3>優待区分（年${x.annual_occurrences}回）</h3><div class="tiers">${x.benefit_tiers.map(t=>`<div class="tier"><div><b>${t.shares==null?'必要株数 未確認':t.shares+'株以上'}</b><br>${t.description}</div><div>${t.annual_value_yen==null?'金額換算対象外':'年間 ¥'+yen.format(t.annual_value_yen)}</div></div>`).join('')}</div>${x.official_source_url?`<a class="official" href="${x.official_source_url}" target="_blank" rel="noopener">公式情報を確認 ↗</a>`:'<span>公式情報URL 未確認</span>'}`;$('detail').showModal()}
  ...
  Promise.all(['benefits.json','market-data.json','verification-queue.json','listed-companies.json','discovery-progress.json'].map(name=>fetch('data/'+name).then(r=>r.json()))).then(([benefits,market,queue,master,progress])=>{items=benefits.map(b=>enrich(b,market[b.code]));[...new Set(items.flatMap(x=>x.record_months||[]))].sort((a,b)=>a-b).forEach(v=>$('month').add(new Option(v+'月',v)));[...new Set(items.map(x=>x.category).filter(Boolean))].sort().forEach(v=>$('category').add(new Option(v,v)));$('confirmedCount').textContent=items.filter(x=>x.benefit_status==='official_confirmed').length+'社';$('candidateCount').textContent=queue.filter(x=>x.result!=='failed').length+'社';$('abolishedCount').textContent=items.filter(x=>x.benefit_status==='abolished').length+'社';$('unresearchedCount').textContent=master.filter(x=>!items.some(i=>i.code===x.code)&&!progress.processed_codes?.includes(x.code)).length+'社';$('failedCount').textContent=progress.failed_codes?.length+'社';const valid=items.filter(x=>x.totalYield!=null);$('avgYield').textContent=valid.length?(valid.reduce((s,x)=>s+x.totalYield,0)/valid.length).toFixed(2)+'%':'—';$('queue').innerHTML=queue.length?queue.slice(0,50).map(x=>`<div class="queue-row"><b>${x.code||''} ${x.name||''}</b><span>${(x.record_months||[]).join('・')||'権利月 未確認'}</span><span>${x.result==='failed'?'取得失敗':'確認待ち'}</span><span>信頼度 ${x.confidence_score??'未取得'}</span><span>${(x.verification_reasons||[]).join(' / ')||'理由 未登録'}</span><span>${x.error_reason||x.evidence_text||'根拠 未取得'}</span></div>`).join(''):'<p>確認待ちはありません。</p>';render()}).catch(()=>{$('empty').hidden=false;$('empty').querySelector('b').textContent='データを読み込めませんでした';$('queue').textContent='確認待ちデータを読み込めませんでした'});
