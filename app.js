(function (root) {
  ...
  const filterBenefits = (items, filters) => items.filter(x => {
    ...
    return (!q || `${x.code} ${x.name} ${x.tier.description}`.toLowerCase().includes(q)) &&
      ...
      (filters.showAbolished || x.benefit_status !== 'abolished') && (!filters.confirmedOnly || x.benefit_status === 'official_confirmed') && (!filters.favoritesOnly || filters.favorites.includes(x.code));
  });
  ...
  const filters = () => ({search:$('search').value.trim(),month:$('month').value,category:$('category').value,maxInvestment:$('maxInvestment').value,hundredOnly:$('hundredOnly').checked,longTermOnly:$('longTermOnly').checked,showAbolished:$('showAbolished').checked,confirmedOnly:$('confirmedOnly').checked,favoritesOnly,favorites});
  ...
  const priceLabel = x => x.price==null?'株価 未確認':`${x.priceSample ? 'サンプル／参考株価' : '株価'} ¥${yen.format(x.price)}`;
  ...
  ['search','month','category','maxInvestment','sort','hundredOnly','longTermOnly','showAbolished','confirmedOnly'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',render));
  $('reset').onclick=()=>{['search','month','category','maxInvestment'].forEach(id=>$(id).value='');$('sort').value='total-desc';$('hundredOnly').checked=$('longTermOnly').checked=$('showAbolished').checked=$('confirmedOnly').checked=false;render()};
  ...
  Promise.all([fetch('data/benefits.json').then(r=>r.json()),fetch('data/market-data.json').then(r=>r.json())]).then(([benefits,market])=>{items=benefits.map(b=>enrich(b,market[b.code]));[...new Set(items.flatMap(x=>x.record_months))].sort((a,b)=>a-b).forEach(v=>$('month').add(new Option(v+'月',v)));[...new Set(items.map(x=>x.category))].sort().forEach(v=>$('category').add(new Option(v,v)));$('companyCount').textContent=items.length+'社';$('confirmedCount').textContent=items.filter(x=>x.benefit_status==='official_confirmed').length+'社';$('candidateCount').textContent=items.filter(x=>x.benefit_status==='candidate').length+'社';$('abolishedCount').textContent=items.filter(x=>x.benefit_status==='abolished').length+'社';const valid=items.filter(x=>x.totalYield!=null);$('avgYield').textContent=(valid.reduce((s,x)=>s+x.totalYield,0)/valid.length).toFixed(2)+'%';render()}).catch(()=>{$('empty').hidden=false;$('empty').querySelector('b').textContent='データを読み込めませんでした'});
