...
class Tests(unittest.TestCase):
 ...
 def test_benefit_master_schema_and_kddi_orix(self):
  ...
  published_items=json.loads(Path('data/benefits.json').read_text())
  self.assertEqual(len(items),len(published_items))
  self.assertEqual(sum(item['benefit_status']=='official_confirmed' for item in items),sum(item['benefit_status']=='official_confirmed' for item in published_items))
 ...
 def test_universe_merge_is_unique_and_preserves_official(self):
  with tempfile.TemporaryDirectory() as directory:
   ...
   source_items=json.loads(Path('data/benefits.json').read_text())
   self.assertEqual(len(items),len(source_items)); self.assertEqual(len(items),len({x['code'] for x in items})); self.assertEqual(q,[])
 ...
 def test_no_dummy_candidates_and_status_counts_are_preserved(self):
  ...
  self.assertTrue({'7550','7616','7412'} <= {x['code'] for x in items if x['benefit_status']=='official_confirmed'})
 ...
 def test_kyokuyo_is_confirmed_and_removed_only_from_verification_queue(self):
  ...
  # Unverified outcomes are kept out of the dashboard verification queue.
