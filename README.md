...
`selection_diagnostic` は実際の `listed-companies.json` から最低5社を選択した時点で終了し、OpenAI APIキーの検査・
公式サイト取得・OpenAI API呼び出し・データ更新・コミットを行わない無料の選択経路診断です。
...
日本株の株主優待について、優待内容・必要株数・権利月・長期保有条件・公式確認状態を比較する静的なPWAです。既存の株主優待データ230社（公式確認済み228社を含む）を収録しています。株価情報は表示せず、投資判断には利用できません。
